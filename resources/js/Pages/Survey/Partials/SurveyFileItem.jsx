...
const SurveyFileItem = ({ fileData, handleDeleteShow }) => {
    return (
        <div className="flex items-center justify-between p-3 bg-gray-50 rounded hover:bg-gray-100 transition-colors">
            <div className="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-500">
                        {fileData?.type} - {fileData && getFileSize(fileData.size)} - {getDateString(fileData.created_at)}
                    </p>
                </div>
            </div>
            <div className="flex space-x-2">
                ...
                <IconButton color="error" variant="contained" size="small" onClick={() => handleDeleteShow(fileData?.survey_file_id)}><Delete /></IconButton>
            </div>
        </div>
    );
};

export default SurveyFileItem;
