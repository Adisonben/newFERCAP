...
import JoinSurveyCalendar from "./Partials/JoinSurveyCalendar";
import AuthenticatedLayout from "@/Layouts/AuthenticatedLayout";
import { Head } from "@inertiajs/react";
import RespAlert from "@/Components/RespAlert";
import AvailableSurveyTable from "./Partials/AvailableSurveyTable";
import { CalendarMonth, TableRows } from "@mui/icons-material";
import { Button } from "@mui/material";

const AvailableSurveyPage = ({ resFormBack, surveys }) => {
    ...
    const eventList = surveys.map((survey) => {
        return {
            ...
            color: survey.is_surveyor?.status
                ? "green"
                : "grey",
            description: survey.description,
            status: survey.is_surveyor?.status || false,
        };
    });
    useEffect(() => {
                ...
                <div className="p-6 text-gray-900 dark:text-gray-100">
                    <div className="flex justify-between mb-4">
                        <p className="text-xl font-bold">
                            Avaliable Surveys{" "}{showCalendar ? "(Calendar)" : "(Table)"}
                        </p>
                    ...
                    <div className="overflow-x-auto">
                        {showCalendar ? (
                            ...
                            <AvailableSurveyTable surveys={surveys} />
                        )}
                    </div>
                </div>
            </div>
...
export default AvailableSurveyPage;
